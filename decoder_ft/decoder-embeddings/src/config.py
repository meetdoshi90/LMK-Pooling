...
class Arguments(TrainingArguments):
    ...
    train_data_config: Optional[str] = field(
        default=None, metadata={"help": "Json file with the train files and their weights"}
    )
    val_data_config: Optional[str] = field(
        default=None, metadata={"help": "Json file with the train files and their weights"}
    )
    ...
    cache_minibatch_size: Optional[int] = field(
        default=None, 
        metadata={"help": "minibatch size for forward pass"}
    )
    
    document_cache_minibatch_size: Optional[int] = field(
        default=None, 
        metadata={"help": "minibatch size for documents during forward pass"}
    )

    use_reranker_for_decoder_distillation: Optional[str] = field(
        default=None, 
        metadata={"help": "use reranker to get relevance signals during decoder training"}
    )
    
    self_distil_recursive: Optional[bool] = field(
        default=False,
        metadata={"help": "if true, we will recursively stack transformer layers (twice) for self distillation"}
    )
    
    self_distil: Optional[bool] = field(
        default=False,
        metadata={"help": "if true, L1 loss will be applied for self distillation"}
    )
    self_distil_src_layer: Optional[int] = field(
        default=None,
        metadata={"help": "if self_distil==true, self_distil_src_layer will be used as teacher representations"}
    )
    self_distil_tgt_layer: Optional[List[int]] = field(
        default=None,
        metadata={"help": "if self_distil==true, self_distil_tgt_layer will be used as student representations"}
    )
    
    freeze_layers_upto: Optional[int] = field(
        default=None,
        metadata={"help": "freeze upto these layers"}
    )
    multi_layer_loss: Optional[bool] = field(
        default=False,
        metadata={"help": "if true, CE loss will be applied at multiple layers"}
    )
    multi_layer_loss_layers: Optional[List[int]] = field(
        default=None,
        metadata={"help": "if multi_layer_loss==true, multi layer loss will be applied at these layers else at all layers"}
    )
    multi_layer_loss_scale: Optional[str] = field(
        default=None, #'layer_scale' or 'uniform'
        metadata={"help": "scaling of multi layer loss"}
    )

    width_compression: Optional[bool] = field(
        default=False,
        metadata={"help": "if true, Width compression across sequence length will be applied"}
    )
    width_compression_layers: Optional[List[int]] = field(
        default=None,
        metadata={"help": "if width_compression==true, width_compression will be applied at these layers else at all layers"}
    )
    width_compression_ratio: Optional[int] = field(
        default=None, 
        metadata={"help": "number of tokens to pool for width compression"}
    )
    width_compression_pooling: Optional[str] = field(
        default='mean', #sum, max
        metadata={"help": "pooling operation for width compression"}
    )


    freeze_pos_emb: bool = field(
        default=False,
        metadata={"help": "if true, freeze the positional embeddings during training"}
    )
      
    no_titles: bool = field(
        default=False,
        metadata={"help": "if true, omit titles from tokenizer (to match SenteneceTransformers)"}
    ...
    pooling_source: str = field(
        default='cls',
        metadata={"help": "compare <CLS> tokens, or compare mean-pooled",
                "choices":["cls", "mean", "last", "lmk"]
        }  
    )
    teacher_pooling_source: str = field(
        default='mean',
        metadata={"help": "compare <CLS> tokens, or compare mean-pooled",
    ...
    # ── LMK Pooling ────────────────────────────────────────────────────────────────
    lmk_granularity: int = field(
        default=64,
        metadata={"help": "Insert one [LMK] token every this many real tokens. "
                        "Used when pooling_source='lmk'."}
    )
    lmk_granularity_set: Optional[str] = field(
        default=None,
        metadata={"help": "Comma-separated list of granularities to sample from randomly "
                        "per sequence, e.g. '32,64,128,256'. Enables variable-granularity "
                        "training. Overrides lmk_granularity when set."}
    )
    lmk_token_id: int = field(
        default=-1,
        metadata={"help": "Token ID of the [LMK] special token. Filled in automatically "
                        "by setup_lmk_token(); keep default -1 unless you know what you're doing."}
    )
    ...
    teacher: str = field(default=None, metadata={"help": "knowledge distillation, compute teacher's output on the fly."})
    direct_similarity_distill: Optional[bool] = field(
        default=False,
        metadata={'help': 'direct distill cos similarity scores instead of its distribution'}
    )
    use_tokenized_data: bool = field(
        default=False,
        metadata={"help": "if true, use pre-tokenized data, only works for distillation for now"}
    )
    save_and_reload_student_only: bool = field(
        default=False,
        metadata={"help": "to save memory, using saving and reloading a checkpint, only load the student part"}
    )
