...
def _get_examples(q_prefix, d_prefix, r_prefix, features):
    query_examples = [{k[len(q_prefix):]: v for k, v in f.items() if k.startswith(q_prefix)} for f in features]
    reranker_examples = _unpack_doc_values([{k[len(r_prefix):]: v for k, v in f.items() if k.startswith(r_prefix)} for f in features])
    ...
    assert len(doc_examples) % len(query_examples) == 0, \
        '{} doc and {} queries and {} reranker'.format(len(doc_examples), len(query_examples), len(reranker_examples))
    if reranker_examples!=None:
        assert len(reranker_examples) == len(doc_examples) * len(query_examples), \
            '{} doc and {} queries and {} reranker'.format(len(doc_examples), len(query_examples), len(reranker_examples))
    return query_examples, doc_examples, reranker_examples