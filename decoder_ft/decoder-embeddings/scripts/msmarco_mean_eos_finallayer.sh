...
TRAIN_DATA_CONFIG=/proj/embedding/meet/Landmark/inf-dds/decoder_ft/decoder-embeddings/msmarco_config.json
...
DATASETS_DIR=/dccstor/irl-rag/meet/data/decoder_embedding/bge-en-icl/merged_msmarco_psg/

MODEL_NAME_OR_PATH=ibm-granite/granite-4.1-8b #mistralai/Mistral-7B-Instruct-v0.3
...
POOLING_SOURCE=${1:-mean}

OUTDIR_BASE="/dccstor/sdg/meet/models/decoder_embedding/lmk_exps/msmarco_bge_en_icl_kd/${MODEL_NAME}"
OUTDIR="${OUTDIR_BASE}/loss_${MULTI_LAYER_LOSS}_layers_${LAYERS}_scale_${SCALE}_nnodes_${NNODES}_gpus_${GPUS_PER_NODE}_temp_${TEMP}_lr_${LR}_wd_${WD}_pool_${POOLING_SOURCE}_lmkexps/"
mkdir -p "${OUTDIR}"
