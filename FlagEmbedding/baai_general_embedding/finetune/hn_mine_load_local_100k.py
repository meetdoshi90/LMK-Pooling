...
def find_knn_neg(model, input_file, candidate_pool, output_file, sample_range, negative_number, use_gpu):
    ...
    # with open("data/finetune/100k/corpus_query_train_data.pkl", "wb") as fOut:
    #     pickle.dump({'corpus': corpus, 'query': queries,'train_data':train_data}, fOut)
    with open("data/finetune/100k/corpus_query_train_data.pkl","rb") as fin:
    ...
    # with open("data/finetune/74m/p_vecs_q_vecs_100k.pkl", "wb") as fOut:
    #     pickle.dump({'corpus': p_vecs, 'query': q_vecs}, fOut)
    with open("data/finetune/100k/p_vecs_q_vecs_v100.pkl","rb") as fin:
    ...
    try:
        ...
        faiss.write_index(index_cpu, "data/finetune/index/corpus_v100_clean.index")
        print("index written")
    except:
        print("failed")
