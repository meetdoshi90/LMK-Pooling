...
def main():
    ...
    out_file = "./dev_losses.txt"
    
    check_file = os.path.isfile(out_file)
    
    if check_file:
        print("Dev loss file already exists appending it to that")
        out_fp = open(out_file,'a')
    else:
        print("Dev loss file does not exist creating a new file")
        out_fp = open(out_file,"w")
        
        
    output = trainer.evaluate(eval_dataset=val_dataset)
    
    out_fp.write(model_args.model_name_or_path)
    out_fp.write("\n")
    out_fp.write("Eval Loss :"+ str(output["eval_loss"]))
    out_fp.write("\n")
    print(output)
    #trainer.save_model()
